...
Original file is located at
    https://colab.research.google.com/drive/15LFl5qMG9yY2-rLd9F5PtsXDwgMCIjlK
...
# todo start #
    YourFilePath = 'comp2211/PA1'# Fill in the missing code/s #
# todo end #
...
def z_score_normalization(input_array):
  ...
  sd = np.std(input_array, axis = 0)
  normalized_array = (input_array - mean)/sd
  # todo end #
...
def manhattan_distance(X_train, X_test):
  ...
  # todo start #
  distance = np.absolute(X_test[:, np.newaxis, : ] - X_train)
  distance = np.sum(distance, axis = 2).reshape(len(X_test[ : , 0]), len(X_train[: , 0]))
  #check later
  # todo end #
  return distance
...
def cosine_distance(X_train, X_test):
  ...
  # X_test: numpy array of shape (num_rows_test, num_features)
  # todo start #
  numerator = np.dot(X_test, X_train.T)
  denominator = np.sqrt(np.sum(np.square(X_train), axis = 1)) * np.sqrt(np.sum(np.square(X_test), axis = 1)[:, np.newaxis])
...
def knn_prediction(distances, y_train, k):
  ...
  # todo start #
  y_classes = np.arange(max(y_train))+1
  sort = np.argsort(distances, axis = 1)[:, 0:k]
  data = np.take(y_train, sort)
  count = np.sum((data[:, :, np.newaxis] == y_classes).astype(int), axis = 1)
  prediction = np.argmax(count, axis = 1)+1
  #to check uniqueness
  unique = np.sort(count, axis = 1)[:, -2:]
  for i in range(len(prediction)):
    if (unique[i, -1] == unique[i, -2]): #if tie
      #distance = 1/distances[i, :]
      #sort_dist = np.sort(distance)[ -k:]
      print("unique", unique[i])
      distance = 1/(np.sort(distances[i, :])[0:k])
      count_i = np.sum(((data[i, :, np.newaxis] == y_classes).astype(int))*distance[:, np.newaxis] , axis = 0)
      #to rule out those that are not max
      count_int = count_i*((count[i, :] == unique[i,-1]).astype(int))
      print("distances", np.sort(distances[i, :])[0:k], "distance", distance, "count_i", count_i, "count_int", count_int, "count", count[i, :], "data", data[i,:])
      prediction[i] = np.argmax(count_int)+1
...
def f_score(y_test, prediction, y_classes):
  ...
  for i in range(num_classes):
      ...
      TP = np.dot((prediction == 1+i).astype(int), (y_test == 1+i).astype(int).T)
      FP = np.dot((prediction == 1+i).astype(int), (y_test != 1+i).astype(int).T)
      FN = np.dot((prediction != 1+i).astype(int), (y_test == 1+i).astype(int).T)
...
def centroid_euclidean_distance(X_train, centroids):
  ...
  # todo start #
  distance = np.sqrt(np.sum(np.square(X_train[:, np.newaxis, :] - centroids), axis = 2)).T
...
def calculate_centroids(X_train, assignment, k):
  ...
  # todo start #
  #print(X_train.shape, assignment.shape)
  bin_count = (np.arange(k)[:, np.newaxis] == assignment).astype(int)
  sorted_count = np.sum(bin_count[:,:, np.newaxis] * X_train, axis = 1)
  frequency = np.sum(bin_count, axis = 1)
  new_centroids = sorted_count/(frequency[:, np.newaxis])
  # todo end #
...
def k_means_cluster_reassignment(X_train, initial_centroids, max_iterations=100):
  ...
  for iteration in range(max_iterations):
    ...
    # todo start #
    distance_2 = centroid_euclidean_distance(X_train, centroids)
    assignment = cluster_assignment(distance_2)
    new_centroids = calculate_centroids(X_train, assignment, k)
...
def k_means_centroid_value(X_train, initial_centroids, max_iterations=100, threshold_value=0.0001):
  ...
  for iteration in range(max_iterations):
    ...
    assignment = cluster_assignment(distance_2)
    new_centroids = calculate_centroids(X_train, assignment, k)
    if np.all(np.abs(new_centroids - centroids) < threshold_value) :
      break
    else:
      centroids = new_centroids
    # todo end #
