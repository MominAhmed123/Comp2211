...
Original file is located at
    https://colab.research.google.com/drive/1OOvD_6fEi7R_MZkjbd2yoIONOKBl7xox
...
if __name__ == '__main__':
  ...
  #print(len(train_url), train_x[0:5], train_y[0:5], len(test_url), len(test_x), len(test_y), len(bboxes))
  #print(bboxes[0:5])
...
def create_model(input_shape=(224, 224, 3)):
  ...
  ###### TO DO ######
  model_x = Conv2D(filters = 128, kernel_size = (3,3), padding = "same", activation = 'relu')(model_x)
  ###################
  ...
  ###### TO DO ######
  model_x = Dense(units = 4, activation = 'sigmoid')(model_x)
  ###################
...
def se_func(x1_pred, y1_pred, w_pred, h_pred, x1_gt, y1_gt, w_gt, h_gt):
  ...
  ###### TO DO ######
  #print(h_pred, h_gt)
  SE = tf.square(x1_pred - x1_gt) + tf.square(y1_pred + h_pred - y1_gt - h_gt) + tf.square(x1_pred + w_pred - x1_gt - w_gt) + tf.square(y1_pred - y1_gt)
  return SE
  ###################
...
def giou_func(x1_pred, y1_pred, w_pred, h_pred, x1_gt, y1_gt, w_gt, h_gt):
  ...
  x1i = tf.maximum(x1_pred, x1_gt)
  y1i = tf.maximum(y1_pred, y1_gt)
  x2i = tf.minimum(x1_pred + w_pred, x1_gt + w_gt)
  y2i = tf.minimum(y1_pred + h_pred, y1_gt + h_gt)
  wi = tf.maximum(0.0, x2i - x1i)
  hi = tf.maximum(0.0, y2i - y1i)
  area_i = wi * hi
  ...
  x1c = tf.minimum(x1_pred, x1_gt)
  y1c = tf.minimum(y1_pred, y1_gt)
  x2c = tf.maximum(x1_pred + w_pred, x1_gt + w_gt)
  y2c = tf.maximum(y1_pred + h_pred, y1_gt + h_gt)
  wc = tf.maximum(0.0, x2c - x1c)
  hc = tf.maximum(0.0, y2c - y1c)
  area_c = wc * hc
  giou = iou - ((area_c - area_u) / (area_c + 1e-7))
...
def extract_failure_case(loss_function, pred, gt, k=5):
  ...
  for i in np.arange(pred.shape[0]):
    loss[i] = loss_func(pred[i], gt[i])
  loss = tf.convert_to_tensor(loss, dtype = tf.float32)
  result = tf.math.top_k(loss,k=k)
  return result.indices.numpy()
  ###################
  pass
